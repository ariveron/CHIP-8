...
int main(int argc, char** argv)
{
    ...
    if (c8_host_init(L"CHIP-8", 64, 32, 8) == 0)
    {
    ...
    uint64_t rateCPU = 18'518;      // 540 Hz
    uint64_t rateIO = 83'333;       // 120 Hz
    uint64_t rateTimers = 166'667;  //  60 Hz
    ...
    while (keepGoing)
    {
        ...
        {
            // Input
            keepGoing = c8_host_is_not_quit();
            c8_update_keys(&vm, c8_host_get_keys());
            rateCPU -= 1'000ULL * c8_host_is_speed_up() * (rateCPU > 5'000);
            rateCPU += 1'000ULL * c8_host_is_speed_down() * (rateCPU < 300'000);
            if (c8_host_is_speed_reset()) rateCPU = 20'000;
